...
- The actual Python package lives under `src/app/REDACTED/`, which is a non-standard layout: it is copied wholesale to `/opt/REDACTED/` in the `Dockerfile` and run via `PYTHONPATH=/opt/REDACTED python3 -m REDACTED`.
